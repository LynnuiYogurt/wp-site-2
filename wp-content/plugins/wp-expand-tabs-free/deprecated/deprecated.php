	...
	return '
        <script>
            jQuery(document).ready(function($){
                $("#tr-easy-tabs-wrapper' . esc_attr( $id ) . ' .tr-easy-tabs-activate").pwstabs({
                    effect: "' . esc_attr( $effect ) . '", 
                    defaultTab: ' . esc_attr( $open ) . ',    
                    containerWidth: "' . esc_attr( $width ) . '",
                    tabsPosition: "' . esc_attr( $position ) . '",   
                    horizontalPosition: "' . esc_attr( $hposition ) . '",
                    verticalPosition: "' . esc_attr( $vposition ) . '",     
                    rtl: ' . esc_attr( $rtl ) . ' 
                });
            });
        </script>
        <style>
            #tr-easy-tabs-wrapper' . esc_attr( $id ) . ' div.pws_tabs_container ul.pws_tabs_controll li a:before, #tr-easy-tabs-wrapper' . esc_attr( $id ) . ' div.pws_tabs_container ul.pws_tabs_controll li a {background-color:' . esc_html( $background ) . '}
            #tr-easy-tabs-wrapper' . esc_attr( $id ) . ' div.pws_tabs_container ul.pws_tabs_controll li a.pws_tab_active:before, #tr-easy-tabs-wrapper' . esc_attr( $id ) . ' div.pws_tabs_container ul.pws_tabs_controll li a.pws_tab_active {background-color:#fff;color:#333}
            #tr-easy-tabs-wrapper' . esc_attr( $id ) . ' div.pws_tabs_container ul.pws_tabs_controll li a {color:' . esc_html( $color ) . '}
            #tr-easy-tabs-wrapper' . esc_attr( $id ) . ' div.pws_tabs_container ul.pws_tabs_controll li a.pws_tab_active, #tr-easy-tabs-wrapper' . esc_attr( $id ) . ' div.pws_tabs_list {border-color:' . esc_html( $border ) . '}
		</style>
        <div id="tr-easy-tabs-wrapper' . esc_attr( $id ) . '" class="tr-easy-tabs-wrapper">
...
function tr_easy_tabs_pro_items( $atts, $content = null ) {
	...
	wp_enqueue_style( 'tr-easy-tabs-pro-main-css' );
	return '
	<div data-pws-tab="tab' . esc_attr( $id ) . '" data-pws-tab-name="' . esc_attr( $title ) . '">
		' . do_shortcode( $content ) . '
	</div>
    ';
}
...
function tr_easy_tabs_pro_items_shortcode( $atts ) {
	...
	while ( $q->have_posts() ) :
		...
		$list .= '
          <div data-pws-tab="tab' . esc_attr( $idd ) . '" data-pws-tab-name="' . do_shortcode( get_the_title() ) . '">
