	...
	class SP_WP_TABS_Field_wp_editor extends SP_WP_TABS_Fields {
		...
		public function render() {
			...
			echo ( wptabspro_wp_editor_api() ) ? '<div class="wptabspro-wp-editor" data-editor-settings="' . esc_attr( wp_json_encode( $editor_settings ) ) . '">' : '';
      		// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			echo '<textarea name="' . esc_attr( $this->field_name() ) . '"' . $this->field_attributes( $attributes ) . $editor_height . '>' . wp_kses( format_for_editor( $this->value ), apply_filters( 'sp_tabs_description_allow_tags', $allowed_tags ) ) . '</textarea>';
